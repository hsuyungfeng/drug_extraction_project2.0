# 药物信息提取项目 - Qwen-Agent 整合方案

## 📋 项目状态
✅ **Qwen-Agent 整合完成** - 成功使用本地 Ollama 模型进行药物信息提取
✅ **API 端点配置正确** - 使用 OpenAI 兼容端点 (http://localhost:11434/v1)
✅ **输出格式验证** - 与目标文件格式完全一致
✅ **完整药物信息提取** - 成功提取适应症、用法用量、注意事项

## 项目概述

本项目旨在使用 Qwen-Agent 框架来自动化提取台湾健保用药品项中的关键信息，包括：
- **适应症** (Indications)
...
- 包含约 14,621 笔药物数据
- 目前已有 3,150 笔完整数据，11,471 笔不完整数据
...
2. **本地 LLM (Ollama/gpt-oss:20b) (专业工作者)**
   - 负责文本处理和结构化信息提取
   - 处理 TFDA/NHI 等官方数据源
   - 使用 OpenAI 兼容 API 端点: `http://localhost:11434/v1`

### 两阶段处理流程

#### 阶段一: Google 搜索信息提取 (由 Qwen-Agent 执行)
- 读取 `uncomplete-research.csv`
- 对每种药物进行 Google 搜索
- 解析搜索结果中的 AI 摘要
- 生成 `google_search_results.csv`
